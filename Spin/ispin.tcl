...
set xversion "iSpin Version 1.0.6 -- 24 October 2011"
...
#or:	set DOT     "C:/Program Files/Graphviz2.26.3/bin/dot"
...
proc cleanup {} {
	...
	catch { eval exec $RM $Fname.nvr spinbat.bat dot.tmp pan.tmp }
...
proc find_field {fld ln} {
	...
	set a [string first "$fld" $ln]; incr a [string length "$fld"]
...
proc display_graph {A} {	;# read output of ./pan -D | dot > dot.tmp
	global fg xoffset yoffset
	...
	while {[gets $fd line] > -1} {
		...
		if {[string first "digraph $A" $line] >= 0} {
			set found 1
			continue
		}
		if {$found == 0} {
		...
		set fc "black"
		set aa [string first "color=" $line]
		if {$aa > 0} {
			incr aa 6
			set bb [string first ", " [string range $line $aa end]]
			set fc [string range $line $aa [expr $aa + $bb - 1]]
		}

		if {[string first " -> " $line] >= 0} {
			set aa	[string first "label=" $line]; incr aa 6
			if {[string range $line $aa $aa] == "\""} {
				set bb	[string first "\", " [string range $line $aa end]]
			} else {
				set bb	[string first ", " [string range $line $aa end]]
				incr bb -1
			}
			set nl	[string range $line $aa [expr $bb + $aa]]	;# newlabel
			set nl	[string trim $nl "	 \"\{\}"]

			set a	[string first "pos=\"" $line]; incr a 7
			set b	[string first "\"" [string range $line $a end]]
			set b	[expr $a + $b - 1 ]
			set tp [string range $line [expr $a - 2] [expr $a - 2]]	;# e or s

			set line [string range $line $a $b]	;# coordinates for spline
			set k [split $line " "]
			set j [llength $k]

			set j2 [expr $j/2]
			set aa [string first "." $j2]
			if {$aa > 0} {
				set j2 [string range $j2 0 [expr $aa - 1]]
			}

			set coords ""
			if {$tp == "e"} {
				set ends "last"
				scan [lindex $k 0] "%f,%f" x y
				set lastpoint "[expr $xoffset + $x] [expr $yoffset + $Maxy - $y]"

				for {set i 1} {$i < $j} {incr i} {
					scan [lindex $k $i] "%f,%f" x y
					set coords " $coords[expr $xoffset + $x] [expr 50 + $Maxy - $y] "
					if {$i == $j2} {	;# midpoint
						$fg create text \
							[expr 25 + $xoffset + $x] [expr $yoffset + $Maxy - $y] \
							-fill white -text "$nl" -tags graph
				}	}
				set coords " $coords$lastpoint "
			} else {
				set ends "first"
				for {set i 0} {$i < $j} {incr i} {
					scan [lindex $k $i] "%f,%f" x y
					set coords " $coords[expr $xoffset + $x] [expr $yoffset + $Maxy - $y] "
					if {$i == $j2} {
						$fg create text \
							[expr 25 + $xoffset + $x] [expr $yoffset + $Maxy - $y] \
							-fill white -text "$nl" -tags graph
			}	}	}
			eval $fg create line $coords -arrow $ends -smooth yes -fill gold -tags graph
			continue
		}
		...
		if {[string first "graph " $line] >= 0} {
			...
			set aa	[string first "label=" $line]
			if {$aa > 0} {
				incr aa 6
				set bb	[string first ", " [string range $line $aa end]]
				incr bb [expr $aa - 1]
				set nl	[string range $line $aa $bb]	;# newlabel
			} else {
				set aa [string first " " $line]
				set nl [string range $line 0 $aa]
			}
			set nl [string trim $nl "	 \""]
			...
			set w [find_field "width="  $line]
			set h [find_field "height=" $line]

			if {[string first "style=dotted" $line] > 0} {
				set extra "-fill lightgreen"
			} else {
				set extra ""
			}
			if {[string first "style=bold" $line] > 0} {
				set extra "-width 4"
			}
			if {[string first "shape=box" $line] > 0} {
				set shape "rectangle"
			} else {
				set shape "oval"
			}

			eval $fg create $shape \
				[expr $xoffset+$x-$w/2] [expr $yoffset + $Maxy - ($y-$h/2)] \
				[expr $xoffset+$x+$w/2] [expr $yoffset + $Maxy - ($y+$h/2)] \
				-outline $fc -fill ivory $extra -tags graph

			$fg create text [expr $xoffset + $x] [expr $yoffset + $Maxy - $y] -text $nl -tags graph
		}
	}
	catch { close $fd }
	if {$found == 0} {
		tk_messageBox -icon info -message "$A not found"
	}
}
...
proc mk_graphs {} {
	...
	# cannot remove dot.tmp at this point -- it's read back in by display_graph
}
